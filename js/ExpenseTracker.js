export default class ExpenseTracker {
    ...
    static html() {
        ...
        <table class="Expense-tracker">
            ...
            <tbody class="entries">
                <tr>
                    <td><input class="input input-data" type="date"></td>
    ...
    save() {
        const data = this.getEntryRows().map(row => {
            return {
                date: row.querySelector(".input-data").value,
    ...
    addEntry(entry = {}) {
        ...
        row.querySelector(".input-data").value = entry.date || new Date().toISOString().split("T")[0];
